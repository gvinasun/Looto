...
const initialArticleShown = 10; // 10 artículos en el inicio
    ...
    const fetchArticles = async () => {
      try {
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/api/articles/my-articles?userId=${userId}`
        );
        ...
        if (response.ok) {
          ...
          const adaptedArticles = data.map((article) => ({
            ...
            state: article.state,
            price: article.price,
            imageUrl: `${process.env.REACT_APP_API_URL}${article.photos[0]}`,
          }));
